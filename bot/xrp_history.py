#!/usr/bin/env python3
"""Historical pattern engine for XRP Bot (KXXRP15M).
...
skipped, not guessed.
  ...
  rounds   : GET /trade-api/v2/markets?series_ticker=KXXRP15M&status=settled
  contract : GET /trade-api/v2/series/KXXRP15M/markets/<T>/candlesticks
             (period_interval=1) -> per-minute yes_bid / yes_ask / volume
  XRP spot : GET api.exchange.coinbase.com/products/XRP-USD/candles
...
HISTFILE = os.path.join(HERE, "record", "xrp_history.json")
...
def _spot_range(start, end, tries=4):
    """1-minute XRP-USD closes, {unix_ts: close}. Coinbase caps a request at 300."""
    out = {}
    for s in range(start, end, 300 * 60):
        e = min(s + 299 * 60, end)
        for a in range(tries):
            try:
                rows = _get("https://api.exchange.coinbase.com/products/XRP-USD/candles",
                            dict(granularity=60,
...
def _row(m, cs, spot):
    ...
    o = _ts(m["open_time"])
    t5 = o + 300
    # OFF-BY-ONE FIX (Sep 15 2026). A Coinbase candle's timestamp is the BUCKET
    # START, so the candle stamped t does not close until t+60. Reading spot[t5]
    # as the minute-5 close gave one free minute of LOOK-AHEAD: that bar closes
    # at minute 6. A live minute-5 lookup can only know the last FULLY CLOSED
    # bar, which is the candle stamped t5-60 (it closes exactly at t5). Proven
    # from real data: Coinbase candle stamped t reproduces the low/high/last of
    # the trades in [t, t+60), and recomputing the stored rows with spot[t5]
    # reproduced all 853 readable stored rsi/mom values exactly.
    last_closed = t5 - 60
    closes = []
    for k in range(20, -1, -1):
        v = spot.get(last_closed - k * 60)
        if v is None:
            return None
        closes.append(v)
    rsi = rsi14(closes)
...
def build(limit=800, existing=None, log=print):
    ...
    return dict(built=time.time(), source=dict(
        rounds="GET /trade-api/v2/markets?series_ticker=KXXRP15M&status=settled",
        contract="GET /trade-api/v2/series/KXXRP15M/markets/<ticker>/candlesticks?period_interval=1",
        spot="GET api.exchange.coinbase.com/products/XRP-USD/candles?granularity=60"),
