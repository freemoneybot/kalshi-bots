...
#    SECONDS of ETHUSDRTI, so the real decision point is minute 14 and a spike in
...
TRAIL_ARM_MULT = 4.0      # "decent profit" on a cheap ticket = 4x the entry
...
def trail_arm_level(entry):
    ...
    return max(entry * TRAIL_ARM_MULT, entry + TRAIL_ARM_MIN)
...
# FINAL 60 SECONDS of ETHUSDRTI against the 60s average before the open, so a
