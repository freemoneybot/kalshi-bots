#!/usr/bin/env python3
"""Eth Bot - KXETH15M. Port of Silver Bot, NORMAL MODE (not scalp mode).

Anthony, Sep 14 2026: "Strike at minute 5" / "No scalp mode." / "Just regular
mode, usually expecting to sell around minute 10-12".

So: the strike is picked and the entry evaluated at MINUTE 5 of each 15-minute
round, the position is then HELD, and it is sold in the minute 10-12 window.
...
import fng                      # crypto Fear & Greed - ETH IS crypto, so this applies
import eth_history as hist      # historical pattern engine (real settled KXETH15M rounds)

RECORD = os.path.join(HERE, "record", "eth.scalp.json")
POSFILE = os.path.join(HERE, "record", "eth.pos.json")
SITE = os.path.join(HERE, "site")
SNAP = os.path.join(SITE, "snapshot.json")
LOG = os.path.join(HERE, "logs", "eth_decisions.log")
KALSHI = "https://api.elections.kalshi.com/trade-api/v2"
# --- THE SETTLEMENT INDEX (Sep 15 2026, correctness fix).
# KXETH15M does NOT settle on Coinbase. Its own rule text from the Kalshi API:
#   "If the simple average of the sixty seconds of CF Benchmarks' ETHUSDRTI
#    before <close> is at least the simple average of the sixty seconds of
#    ETHUSDRTI before <open>, the market resolves to Yes."
# Both ends are 60-SECOND AVERAGES of CF Benchmarks' ETHUSDRTI index. Kalshi
# serves that exact index free and unauthenticated, ~1 tick per second:
KALSHI_EXT = "https://external-api.kalshi.com/trade-api/v2"
IDX_SRC = "kalshi-index"        # ETHUSDRTI, the series the contract settles on
...
MIN_ENTRY = 0.05        # Anthony's hard rule: 5c floor
MAX_ENTRY = 0.80        # Anthony Sep 14 18:14Z: "Look, nothing above 80"
MAX_SPREAD = 0.06
...
# time-based trigger (the minute 10-12 sell window, the window-close flat).
...
MIN_MOM = 0.02            # |3-min ETH momentum| gate, same constant/comparison as silver
...
# --- Trailing lock on winners: once a position is up TRAIL_ARM from entry, a
# trailing stop arms and exits if price falls back by the band for its peak gain.
# This can fire BEFORE the minute 10-12 window - that is the point.
TRAIL_ARM = 0.08          # arm once up 8c from entry (was 15c: it armed too late)
# Giveback scaled to the size of the run. An 8c giveback against a 15c gain hands
# back more than half; measured live on two real rounds (23:50 peaked +19.9 and
# closed -3.0; 02:50 peaked +15.0 and closed +2.0). Bands, by peak gain over entry:
TRAIL_BANDS = ((0.25, 0.07), (0.15, 0.06), (0.08, 0.05))
TRAIL_FLOOR = 0.06        # an armed position never closes below entry + 6c

def trail_band(gain):
    """Giveback allowed from the peak, given how far the peak is above entry."""
    for lo, give in TRAIL_BANDS:
        if gain >= lo:
            return give
    return None
...
# --- SETTLEMENT MECHANIC AND THE EXITS (Sep 15 2026). Settlement is the simple
# average of the FINAL 60 SECONDS of ETHUSDRTI against the 60s average before the
# open, so a spike in the last 30 seconds moves the settled outcome by only about
# half its size, and the effective decision deadline is ~minute 14, not 15.
# MEASURED CONSEQUENCE FOR THIS BOT: none. EXIT_MINUTE_END is 12.0 and
# FORCE_EXIT_SECS puts it flat 40s before the close, so the position is always
# closed by ~minute 12 - roughly two minutes BEFORE the averaging window even
# begins. The hard stop and the trailing lock therefore never run inside the
# settlement average, and no late-noise allowance could change an exit that has
# already happened. So NOTHING about the stop or the trail was changed here; the
# deadline is recorded and published for the page, and that is all it does.
DECISION_DEADLINE_MIN = 15.0 - SETTLE_AVG_SECS / 60.0   # = 14.0
# --- Exit: NORMAL MODE. Hold, then sell in the minute 10-12 window.
EXIT_MINUTE_START = 10.0   # from here, sell as soon as the bid is actually profitable
EXIT_MINUTE_END = 12.0     # at 12.0 sell at whatever the book gives, win or lose
...
def index_ticks(event_ticker, rng="15min"):
    """~1-second ticks of CF Benchmarks' ETHUSDRTI - the series KXETH15M settles
...
def coinbase_candles():
    """1-minute ETH-USD OHLCV, oldest-first. Same source run_bot.py uses for eth."""
    rows = get("https://api.exchange.coinbase.com/products/ETH-USD/candles",
               dict(granularity=60))
    out = []
...
def coinbase_spot():
    j = get("https://api.coinbase.com/v2/prices/ETH-USD/spot")
...
def sentiment():
    """ETH is crypto, so the crypto Fear & Greed index is the sentiment input here.
    ...
    out = dict(source="Crypto Fear & Greed index (alternative.me)",
               note="ETH is crypto, so the crypto Fear & Greed index applies here. "
...
# ----------------------------------------------------------------- record
def blank():
    return dict(system="Eth Bot", series=SERIES, started=time.time(),
...
def read_signals(market):
    ...
    from CF Benchmarks' ETHUSDRTI - the index KXETH15M actually settles on - read
    ...
    try:
        ...
        s["spot_source"] = ("CF Benchmarks ETHUSDRTI (the KXETH15M settlement index) "
        ...
        try:
            ...
            if ref:
                ...
                s["ref_note"] = (f"reference = 60s ETHUSDRTI average before the open "
    ...
    if rows is None:
        ...
        if rows:
            s["price_src"] = CB_SRC
            s["spot_source"] = ("Coinbase ETH-USD 1-min candles (FALLBACK - the "
                                "ETHUSDRTI settlement index could not be read)")
            try:
...
def stop_for(entry):
    ...
    entries were knocked out on noise before the minute 10-12 window opened."""
...
def decide(market, bk, sig, hl=None, hist_built=None):
    ...
    The only four NOTRADE reasons, each logged plainly:
      1. the signal's own side costs more than 80c (or under the 5c floor)
    ...
    if mom is not None:
        if abs(mom) >= MIN_MOM / 100.0:
            ...
            why.append(f"ETH 3-min momentum {mom:+.3f}%")
        else:
            why.append(f"ETH flat ({mom:+.3f}%)")
    r = sig.get("rsi")
    if r is not None:
        if r > 60:
    ...
    if abs(lean) >= 0.01:
        ...
        why.append(f"lean {lean:+.1f} -> {side}")
    ...
    # VETO the signal's own side (no trade) - it can never buy the other one.
    if tape is not None:
    ...
    if abs(lean) < CONVICTION_FLOOR:
        if abs(lean) < HIST_UNLOCK_MIN:
            return None, why + [f"NOTRADE: lean {lean:+.1f} below the "
        ...
        why.append(f"history-unlock: lean {lean:+.1f} is under the {CONVICTION_FLOOR:.1f} floor "
    ...
    if cost > MAX_ENTRY or cost < MIN_ENTRY:
        ...
        return None, why + [f"NOTRADE: signal says {side} but {side} costs "
                            f"{cost*100:.0f}c - outside the {MIN_ENTRY*100:.0f}-"
                            f"{MAX_ENTRY*100:.0f}c ceiling"]
    return side, why
...
def bias_of(sig):
    ...
    if lean is not None and abs(lean) >= 0.01:
        return ("BULL" if lean > 0 else "BEAR"), f"combined lean {lean:+.1f}"
    mom = sig.get("spot_mom_pct")
    if mom:
...
def main():
    ...
    H = hist.load()
    if H:
        log(f"history: {H.get('n')} real settled KXETH15M rounds loaded from record/eth_history.json")
    else:
    ...
    while True:
        ...
        try:
            ...
            if pos:
                if pos["ticker"] != m["ticker"]:
                    ...
                    try:
                        ...
                        ob = book(om)
                        close_position(rec, pos, ob, "window rolled")
                    except Exception as e:
                        close_unavailable(rec, pos, str(e)[:120])
                    pos = None
                else:
                    bid = bk["yes_bid"] if pos["side"] == "UP" else bk["no_bid"]
                    ent = pos["entry"]
                    round_fees = fee(ent) + fee(max(ent, 0.05))
                    tp_bid = ent + round_fees + 0.02
                    ...
                    if bid:
                        if pos.get("peak_bid") is None or bid > pos["peak_bid"]:
                            pos["peak_bid"] = bid
                            save_pos(pos)
                        if not pos.get("trail_armed") and pos["peak_bid"] - ent >= TRAIL_ARM:
                            pos["trail_armed"] = True
                            save_pos(pos)
                            _g = pos["peak_bid"] - ent
                            _b = trail_band(_g)
                            log(f"TRAIL ARMED {pos['side']} {pos['ticker']} minute {minute:.1f} "
                                f"- up {_g*100:.0f}c, peak {c(pos['peak_bid'])}c, "
                                f"band {_b*100:.0f}c giveback, floor {c(ent + TRAIL_FLOOR)}c")
                    reason = None
                    # NORMAL MODE: hold through the middle of the round and sell in
                    # the minute 10-12 window. The hard stop is the only early exit.
                    if pos.get("trail_armed") and bid:
                        pk = pos["peak_bid"]
                        gain = pk - ent
                        band = trail_band(gain) or TRAIL_BANDS[-1][1]
                        floor_px = ent + TRAIL_FLOOR
                        tail = (f"peak {c(pk)}c (gain {gain*100:.0f}c, "
                                f"band {band*100:.0f}c giveback, floor {c(floor_px)}c)")
                        # The floor is checked FIRST and on every tick, and it is
                        # checked against the live BID (what we can actually sell
                        # into), not the peak. Whichever of floor / giveback comes
                        # first ends the position. Honest caveat: we cannot fill
                        # above the best bid - if the bid has already gapped BELOW
                        # the floor, the floor cannot be honoured and the line says
                        # so rather than printing a floor that was never applied.
                        gapped = bid < floor_px
                        hit_floor = bid <= floor_px
                        hit_band = (pk - bid) >= band
                        if hit_floor or hit_band:
                            which = ("trailing floor" if hit_floor and not hit_band
                                     else "trailing lock" if hit_band and not hit_floor
                                     else "trailing floor+lock")
                            reason = (f"{which} at minute {minute:.1f} - out at "
                                      f"{c(bid)}c - " + tail)
                            if gapped:
                                reason += (f" floor {c(floor_px)}c NOT HONOURED - best bid "
                                           f"was {c(bid)}c (gapped through the floor)")
                            else:
                                reason += f" floor {c(floor_px)}c honoured"
                    if reason:
                        pass
                    elif bid and ent - bid >= sl_move:
                        reason = f"hard stop at minute {minute:.1f} - cut {sl_move*100:.0f}c below entry"
                    elif minute >= EXIT_MINUTE_END:
                        reason = f"minute {minute:.1f} - sell window closed, out at the book"
                    elif minute >= EXIT_MINUTE_START and bid and bid >= tp_bid:
                        reason = f"minute {minute:.1f} - in profit in the sell window, took it"
                    elif secs_left <= FORCE_EXIT_SECS:
                        reason = "window closing - flat"
                    if reason:
                        action, action_why = "SELL", reason
                    else:
                        action = "HOLD"
                        action_why = (f"holding {pos['side']} from {c(ent)}c - sell window is "
                                      f"minute {EXIT_MINUTE_START:.0f}-{EXIT_MINUTE_END:.0f}, "
                                      f"cut at {c(ent - sl_move)}c")
                    if reason:
                        if bid:
                            close_position(rec, pos, bk, reason)
                        else:
                            close_unavailable(rec, pos, "no bid on the book")
                        pos = None
                        loop_note = reason
                    else:
                        ...
                        dists = []
                        if bid:
                            dists.append(bid - (ent - sl_move))          # hard stop
                            if pos.get("trail_armed"):
                                dists.append(bid - (ent + TRAIL_FLOOR))  # trailing floor
                                _pk = pos.get("peak_bid") or bid
                                _bd = trail_band(_pk - ent) or TRAIL_BANDS[-1][1]
                                dists.append(_bd - (_pk - bid))          # giveback level
                        secs = []
                        secs.append((EXIT_MINUTE_START - minute) * 60.0)
                        secs.append((EXIT_MINUTE_END - minute) * 60.0)
                        ...
                        if near_px or near_t:
                            ...
                            action_why += (f" \u00b7 polling every {HOLD_LOOP}s "
                                           f"(nearest trigger {min(dists)*100:.1f}c away)")
            ...
            if evaluable:
                ...
                w["ticks_in_window"] += 1
                try:
                    side, why = decide(m, bk, sig, hl, (H or {}).get("built"))
                    ok = True
                except Exception as e:
                ...
                if ok and side and m["ticker"] in entered:
                    ...
                    cost = bk["yes_ask"] if side == "UP" else bk["no_ask"]
                    ...
                    log(f"ENTRY {side} {m['ticker']} strike {m.get('floor_strike')} "
                        ...
                        f"signal_side={side} side_taken={side} "
                        f"stop={stop_for(cost)*100:.1f}c "
                        f"src={sig.get('price_src') or 'unavailable'} "
                        f"(cut at {c(cost - stop_for(cost))}c)" + via + " :: " + "; ".join(why))
                elif ok:
                    log(f"NOTRADE {m['ticker']} minute {minute:.1f} "
                        ...
                        f"vol={sig.get('contract_vol_15m')} yes_ask={c(bk['yes_ask'])} "
                        f"no_ask={c(bk['no_ask'])} :: " + (why[-1] if why else "watching"))
                    evaluated.add(m["ticker"])
                ...
                if m["ticker"] in entered:
                    ...
                    loop_note = (f"minute {minute:.1f} - strike is picked at minute "
                                 ...
                                 f"(regular mode, exit window minute {EXIT_MINUTE_START:.0f}-{EXIT_MINUTE_END:.0f})")
                else:
                    loop_note = (f"minute {minute:.1f} - minute-{STRIKE_MINUTE:.0f} strike "
                                 f"window is past for this round (regular mode, exit window minute "
                                 f"{EXIT_MINUTE_START:.0f}-{EXIT_MINUTE_END:.0f})"
            ...
            state = dict(
                system="Eth Bot", asset="eth", label="ETH",
                series=SERIES, tv_symbol="COINBASE:ETHUSD",
                updated=time.time(),
                round=dict(minute=round(minute, 2), strike_minute=STRIKE_MINUTE,
                           sell_window=[EXIT_MINUTE_START, EXIT_MINUTE_END],
                ...
                targets=(dict(take_at=c(pos["entry"] + fee(pos["entry"]) * 2 + 0.02),
                              cut_at=c(pos["entry"] - stop_for(pos["entry"]))) if pos else None),
                rules=dict(entry_band=f"{MIN_ENTRY*100:.0f}-{MAX_ENTRY*100:.0f}c (never above 80c)",
                           strike_timing=f"strike picked / entry evaluated at minute {STRIKE_MINUTE:.0f} of the 15-min round",
                           gate="one call every round at minute 5; the signal picks the side and that side is taken or there is NO TRADE - never the opposite side. Four NOTRADE reasons only: the signal side costs over 80c, the tape vetoes the signal side, the orderbook read fails, or |lean| is below the conviction floor",
                           exits=f"normal mode: hold, sell in the minute {EXIT_MINUTE_START:.0f}-{EXIT_MINUTE_END:.0f} window; hard stop any time",
                           holds=True,
                           ...
                           trailing_lock=f"arms once up {TRAIL_ARM*100:.0f}c from entry; giveback allowed from the peak scales with the run - 8-14c gain: 5c, 15-24c: 6c, 25c+: 7c - and an armed position never closes below entry + {TRAIL_FLOOR*100:.0f}c. This can fire before the minute 10-12 window",
                           ...
                           settlement=(f"KXETH15M settles on the simple average of the final "
                                       f"{SETTLE_AVG_SECS}s of CF Benchmarks' ETHUSDRTI against the "
                                       ...
                                       f"always flat by minute {EXIT_MINUTE_END:.0f}, before that window "
                                       f"opens, so the stop and trail are unchanged by it"),
                           price_series=("all indicators computed from the ETHUSDRTI settlement index "
                                         "(Kalshi live_data, ~1 tick/s); Coinbase spot is a fallback only "
                                         "and every log line names the source"),
                           hard_stop=f"proportional: {STOP_FRAC*100:.0f}% of entry, min {STOP_MIN*100:.0f}c, max {STOP_MAX*100:.0f}c below entry"),
                expected=dict(direction=hl["expected"], hit_rate_pct=hl["hit_rate"],
                              ...
                              source="real settled KXETH15M rounds from the Kalshi API - "
                ...
                anchor=dict(ref60=sig.get("ref60"), ref60_ticks=sig.get("ref60_ticks"),
                            ...
                            rule="settlement compares the final 60s ETHUSDRTI average "
