...
HALF_STOP_FRAC = 0.50     # cheap-side only: cut if the bid trades at or below
HALF_STOP_BY_MIN = 8.0   # no minute cutoff (worker b7d16102, Sep 16): the
                          # independent n=13 sample rose monotonically the LATER
                          # the cutoff, best with none; the 1.5x arm gate already
                          # protects a ticket that ran. The '8' was fitted.    # half the entry BEFORE this minute. See the block in
                          # the position loop for the 25-of-25 evidence.
TRAIL_ARM_MULT = 1.5      # Sep 16 03:20Z: 4.0 -> 2.0 -> 1.5. TWO LIVE ROUNDS
                          # PEAKED JUST UNDER THE 2x ARM AND THEN RETURNED THE
                          # ENTIRE STAKE: 7.5c peaked 14.0c (1.86x, arm 15c) and
                          # 27.0c peaked 51.0c (1.89x, arm 54c) -> exited 0.7c,
                          # -29.3c. The hole is structural, not bad luck: BELOW
                          # THE ARM LEVEL THERE IS NO PROTECTION AT ALL, because
                          # the giveback trail cannot act until it arms and the
                          # hard stop is off in cheap-side mode. So a ticket can
                          # very nearly double and still lose 100%. 1.5x arms
                          # early enough to cover that cluster, and on the
                          # 203-round sim 1.5x (-2.16c/trade) and 2x (-2.23c)
                          # were indistinguishable - so this costs nothing
                          # measurable and closes a real hole.
                          # Sep 16 00:40Z (kept for the record): was 4.0. MEASURED on 203 real XRP
...
TRAIL_GIVEBACK_FRAC = 0.60  # once armed, give back at most 40% of the PEAK GAIN
    ...
    while True:
        ...
        try:
            ...
            if not m:
                ...
                if not nomarket_logged:
                    nomarket_logged = True
                    log("NOMARKET KXXRP15M - Kalshi lists no OPEN round right now "
                        "(future rounds may exist with status 'initialized'). "
                        "Holding; this is the exchange, not a fault. "
                        "One line per gap - the next line is when a round opens again.")
                time.sleep(LOOP)
                continue
            if nomarket_logged:
                nomarket_logged = False
                log(f"MARKETBACK KXXRP15M - an open round is listed again: {m['ticker']}")
            ...
            if pos:
                if pos["ticker"] != m["ticker"]:
                    ...
                    if stop_inert:
                        stop_px = 0.0
                    # THE EARLY-HALVING STOP. Sep 16 04:00Z, from OUR OWN tape
                    # (worker dbc7fd89, 21 recorded KXXRP15M rounds). The cleanest
                    # result in the whole project: once a cheap ticket's bid trades
                    # below HALF of what we paid, EARLY, it is finished.
                    #   halved before minute 8: 0 of 5 recovered to entry
                    #   (minute-5 sample); 0 of 20 on the expanded sample.
                    #   25 of 25 observations, no exceptions.
                    # Adding exactly this stop was worth +3.20c/trade (n=11,
                    # paired p=0.045) and +1.88c/trade (n=83, p=0.0002).
                    # THE TIME LIMIT IS WHAT MAKES IT WORK: an ALWAYS-ON 50% stop
                    # is WORSE than none (-2.41c/trade) because it cuts winners
                    # that dip after arming. Late halvings are winners on their way
                    # down from a peak; early halvings are corpses.
                    # This is also the honest answer to Anthony's own unanswered
                    # question, "cut at 25% of entry" (m:9a341b5b) - his instinct
                    # was right in direction; the tape says half, and only early.
                    # It is a LOSS REDUCTION, not a profit. Sample is 25 paths.
                    half_stop_px = ent * HALF_STOP_FRAC if CHEAP_SIDE_MODE else 0.0
                    ...
                    if reason:
                        pass
                    elif (bid and half_stop_px > 0.0
                          and minute < HALF_STOP_BY_MIN
                          and not pos.get("trail_armed")
                          and bid <= half_stop_px):
                        # Early-halving cut, RE-ENABLED Sep 16 17:56 UTC but ONLY
                        # BEFORE MINUTE 8. This is the best cell in worker 6e994be9's
                        # own table (-0.86c/trade over 31 rounds, vs -2.76c for the
                        # unconditional stop I shipped at 03:00 and -2.15c for no stop
                        # at all). Removing it entirely was an overcorrection: live
                        # trades 37 (-26.9c) and 38 (-28.8c) both rode the full premium
                        # to zero without ever arming, which the minute-8 cut catches. Worker 6e994be9 measured it on 31 distinct
                        # tape rounds: 17 rounds halved while unarmed, 6 came back to
                        # entry and 5 to a post-fee profit, and one ran from a halving
                        # to a 95.9c bid (-17.0c with the stop, +68.9c without).
                        # The "0 of 27 never recovered" finding it shipped on counted
                        # book observations, not rounds. Live -2.76c/trade with it,
                        # -2.15c/trade without (n=31, paired p=0.149).
                        reason = (f"cut at {c(bid)}c - the bid halved to at or below "
                                  f"{c(half_stop_px)}c (50% of the {c(ent)}c entry) "
                                  f"at minute {minute:.1f}, trail not armed, before the "
                                  f"minute-8 cutoff. Measured on 31 distinct tape "
                                  f"rounds: 17 halved unarmed, 6 got back to entry "
                                  f"and 5 to a post-fee profit, so this cut is a "
                                  f"trade-off, not a certainty - but cutting only "
                                  f"before minute 8 was the best rule tested "
                                  f"(-0.86c/trade vs -2.76c unconditional, -2.15c "
                                  f"with no stop)")
                    ...
                    if reason:
                        ...
                        if bid:
                            ...
                            if (half_stop_px > 0.0 and minute < HALF_STOP_BY_MIN
                                    and not pos.get("trail_armed")):
                                # the early-halving cut is a live trigger too, so the
                                # adaptive poll must tighten as the bid approaches it.
                                # Bug #8 was caused by a trigger the cadence block did
                                # not know about; every live level belongs in dists.
                                dists.append(bid - half_stop_px)
                        secs = []
